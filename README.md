# Hi 👋 I'm Nathan Fernandes

**Founder of [SYNN-IA](https://nathanfernandes.fr)** — I design digital products, automations and AI-assisted tools that solve real business problems, from the idea to production.

📍 Dijon, France · 🎓 Bachelor in Management @ BSB · 🌐 **[nathanfernandes.fr](https://nathanfernandes.fr)**

---

## 🚀 What I do

- **Full-stack web development** — React, Next.js, TypeScript, Supabase
- **Process automation & AI integration** — Claude, GPT and related models, always with human validation
- **Product design** — from need analysis to prototype, then to production

> My approach: start from the **business problem**, then choose the technology.

---

## 🛠 Featured projects

| Project | What it is | Status |
| --- | --- | --- |
| **[IA-Muscu — La Forge](https://github.com/SynnIA/ia-muscu)** | AI coaching for training & nutrition (PWA, Claude + vision) · live: [ia-muscu.vercel.app](https://ia-muscu.vercel.app) | 🟢 In production |
| **[CORHOM](https://corhombtp.com)** | Recruitment website + AI-assisted internal tool (GPT) to structure & publish candidate profiles | 🟢 In production, used daily |
| **[ECHO DISTRICT](https://github.com/SynnIA/echo-district)** | Narrative 2D investigation game with generative-AI dialogue NPCs (Phaser + TS, Anthropic SDK) | Playable demo |
| **[Sonic Vision](https://github.com/SynnIA/sonic-vision)** | iOS accessibility app for blind & visually impaired users — LiDAR, haptics, spatial audio, offline | Swift Student Challenge 2026 |
| **[FitTrack](https://github.com/SynnIA/FitTrack)** | Sports & nutrition tracking web app (Next.js / TypeScript / Supabase) | In progress |

More on my portfolio → **[nathanfernandes.fr](https://nathanfernandes.fr)**

---

## 🔒 Also building (private for now)

- **J.A.R.V.I.S** — Windows desktop AI assistant (local wake-word, streaming LLM, animated particle orb) · Electron + React + TS
- **Blind Heist** — asymmetric-perception multiplayer heist game with an authoritative anti-cheat server · Rust + SpacetimeDB + PixiJS
- **AURA** — physical voice assistant running on a Raspberry Pi (on-device wake-word & TTS)
- **GhostWire** — honest R&D on news-sentiment paper-trading (FinBERT) · Python
- **Nexo** — a personal finance hub
- Ongoing R&D at the intersection of **AI and health**

---

## 🧰 Tech

![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-06B6D4?style=flat&logo=tailwindcss&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3FCF8E?style=flat&logo=supabase&logoColor=white)
![Swift](https://img.shields.io/badge/Swift-F05138?style=flat&logo=swift&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Stripe](https://img.shields.io/badge/Stripe-635BFF?style=flat&logo=stripe&logoColor=white)

---

## 🤖 How I work with AI

I use each model for its strengths — **Claude & Claude Code** for development and agents, **ChatGPT & custom GPTs** for business assistants and data structuring, **Perplexity** for sourced research, **Gemini** for multimodal. The right tool for the right job, always with human validation.

---

## 📫 Get in touch

- 🌐 Portfolio — **[nathanfernandes.fr](https://nathanfernandes.fr)**
- 💼 LinkedIn — **[Nathan Fernandes](https://www.linkedin.com/in/nathan-fernandes-synnia/)**
- ✉️ nathan.fernandes@synn-ia.com
